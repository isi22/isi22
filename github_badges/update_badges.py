...
BADGE_BLOCK_PATTERN = re.compile(
    f"{re.escape(START_BADGE_MARKER)}(.*?){re.escape(END_BADGE_MARKER)}", re.DOTALL
)
...
THUMBNAIL_BLOCK_PATTERN = re.compile(
    f"{re.escape(START_THUMBNAIL_MARKER)}(.*?){re.escape(END_THUMBNAIL_MARKER)}",
    re.DOTALL,
)
...
def process_badge_content(content: str) -> Tuple[str, str, bool]:
    ...
    badge_keys = [
        key.strip() for key in match.group(1).strip().split(",") if key.strip()
    ]
...
def parse_project_file(
    content: str, badges_html: str, file_path: Path, repo_name: str
) -> Optional[Dict]:
    ...
    if thumbnail_match:
        ...
        if img_src_match:
            ...
            thumbnail_url = (
                f"https://raw.githubusercontent.com/{repo_name}/main/{relative_path}"
            )
    else:
        thumbnail_url = ""

    if file_path.name == "README.md" and len(file_path.parts) == 1:
        url = f"https://github.com/{repo_name}"
    elif file_path.suffix == ".md":
        url = f"https://github.com/{repo_name}/blob/main/{file_path.as_posix()}"
    elif file_path.suffix == ".ipynb":
        url = (
            f"https://nbviewer.org/github/{repo_name}/blob/main/{file_path.as_posix()}"
        )
...
def process_file(file_path: Path, repo_name: str, is_private: bool) -> Optional[Dict]:
    ...
    try:
        if file_path.suffix == ".md":
            ...
            for cell in notebook_data.get("cells", []):
                if cell.get("cell_type") == "markdown":
                    ...
                    updated_source, badges_html, was_changed = process_badge_content(
                        source_content
                    )
                    if was_changed:
                        cell["source"] = [
            ...
            if notebook_modified:
                file_path.write_text(
                    json.dumps(notebook_data, indent=1, ensure_ascii=False) + "\n",
                    encoding="utf-8",
                )
                print(f"Updated badges in {file_path.name}.", file=sys.stderr)
        else:
        ...
        if not is_private:
            project_data = parse_project_file(
                content, badges_html, file_path, repo_name
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update badges and parse project data."
    )
    parser.add_argument("--repo-name", required=True)
    parser.add_argument(
        "--is-private", required=True, type=lambda v: v.lower() == "true"
    )
